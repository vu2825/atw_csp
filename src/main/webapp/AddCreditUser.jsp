...
.topup-form { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }
.topup-label { font-weight: 600; color: #334155; }
...
body:not(.light-mode) .topup-label { color: #e2e8f0; }
...
@media (max-width: 768px) {
  ...
  .topup-submit { justify-content: center; }
}
...
<body>
	...
	<section class="topup-box">
		<form action="TopUpRequest" method="POST"
			class="topup-form">
			<input name="action" value="AddCredit" class="hidden"/>
			
			<label for="amount" class="topup-label">Yêu cầu nạp tiền</label>
				...
				<table class="table">
					...
					<tbody>
						<c:forEach var="t" items="${topups}">
							...
							<c:if test="${t.status == 'accept'}">
								<c:set var="countAccept" value="${countAccept + 1}" />
							</c:if>
							<c:if test="${t.status == 'discard'}">
